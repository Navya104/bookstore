...
const BooksSingleCard = ({ item }) => {
    ...
    return (
        <div key={item._id} className="border-2 border-gray-500 rounded-lg px-4 py-2 m-4 relative hover:shadow-xl">
            <h2 className="absolute top-1 right-2 px-4 py-1 bg-red-300 rounded-lg">{item.publishedYear}</h2>
            ...
            <div className="flex justify-between items-center gap-x-2 mt-4 p-4">
                ...
                <Link to={`/books/delete/${item._id}`}>
                    <AiOutlineDelete className="text-2xl text-red-600 hover:text-black"></AiOutlineDelete>
                </Link>
            </div>
            {showModal && (<BookModal item={item} onClose={() => setShowModal(false)}></BookModal>)}
        </div>
    )
}
